// 2.2_upload_revit_file.js - COMPLETE SELF-CONTAINED SERVER
// Educational Purpose: Learn how to upload Revit (.rvt) files to APS Object Storage Service
...
const uploadRevitFileHandler = async (req, res) => {
    const { socketId } = req.body;
    const io = req.app.get('io');
    
    console.log('Starting operation: Upload Revit File');
    
    if (io && socketId) {
        io.to(socketId).emit('status', { message: '--- Step: UPLOAD REVIT FILE ---' });
        io.to(socketId).emit('status', { message: 'Starting upload of Revit file...' });
    }
    ...
    try {
        ...
        if (!req.file) {
            return res.status(400).json({
                success: false,
                operation: 'Upload Revit File',
                error: 'No file uploaded',
                details: 'Please select a Revit (.rvt) file to upload',
        ...
        const fileName = 'run.rvt'; // Design Automation expects this name
        ...
        if (io && socketId) {
            io.to(socketId).emit('status', { message: '✅ Revit file uploaded successfully!' });
        }
        ...
        res.status(500).json({
            success: false,
            operation: 'Upload Revit File',
            error: 'Failed to upload Revit file',
...
module.exports = uploadRevitFileHandler;
...
if (require.main === module) {
    ...
    // Set up the route
    app.post('/upload-revit-file', upload.single('revitFile'), uploadRevitFileHandler);
    
    // Start the educational server for Upload Revit File
    app.listen(PORT, () => {
        console.log(`🟫 EDUCATIONAL SERVER: Upload Revit File`);
        console.log(`📡 Running on: http://localhost:${PORT}`);
        console.log(`🎯 Endpoint: POST /upload-revit-file`);
        console.log(`📚 This server demonstrates how to upload Revit (.rvt) files`);
